...
func (n *node) doMatch(fn Operation, r Range, overlapper Overlapper) (done bool) {
	i := sort.Search(len(n.interfaces), func(i int) bool {
		return !overlapper.ToLeft(n.interfaces[i].Range().Start, r.End)
	})
	if len(n.children) == 0 {
		for _, e := range n.interfaces[:i] {
			if overlapper.ToLeft(r.Start, e.Range().End) {
				done = fn(e)
				if done {
					return
				}
	...
	for j := 0; j < i; j++ {
		c := n.children[j]
		if overlapper.ToLeft(r.Start, c.Range.End) {
			done = c.doMatch(fn, r, overlapper)
			if done {
				return
			}
		}
	...
	if overlapper.Overlap(r, n.children[i].Range) {
		done = n.children[i].doMatch(fn, r, overlapper)
	}
	return
}

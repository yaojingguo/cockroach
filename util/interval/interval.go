...
type Overlapper interface {
	...
	// ToLeft returns true if a is the left of b.
	ToLeft(a Comparable, b Comparable) bool
}
...
// ToLeft checks where a is to the left of b in the inclusive way.
func (overlapper inclusiveOverlapper) ToLeft(a Comparable, b Comparable) bool {
...
// ToLeft checks where a is to the left of b in the exclusive way.
func (overlapper exclusiveOverlapper) ToLeft(a Comparable, b Comparable) bool {
	return a.Compare(b) < 0
}
...
func (n *node) getRange(i int16) Range {
	if len(n.children) == 0 {
		return n.interfaces[i].Range()
	}
	if i%2 == 1 {
		return n.interfaces[(i-1)/2].Range()
	}
	return n.children[i/2].Range
}

func (n *node) maxEnd(l, h int16) int16 {
	maxVal := n.getRange(l).End
	maxIdx := l
	for i := l + 1; i <= h; i++ {
		val := n.getRange(i).End
...
func (n *node) maxEnd2(a, b, c int16) int16 {
	x := n.getRange(a).End
	y := n.getRange(b).End
	z := n.getRange(c).End
	if x.Compare(y) >= 0 {
...
func (n *node) rebuild(l, h int16) int16 {
	if h-l+1 <= cutoff {
		return n.maxEnd(l, h)
	}
	m := (l + h) / 2
	leftMax := n.rebuild(l, m-1)
	rightMax := n.rebuild(m+1, h)
	max := n.maxEnd2(leftMax, m, rightMax)
	n.rightmosts[m] = max
	return max
}

func (n *node) allLen() int {
	return len(n.interfaces) + len(n.children)
}

// rebuildRightmosts rebuilds rightmosts for this node.
func (n *node) rebuildRightmosts() {
	length := int16(n.allLen())
	n.rightmosts = n.rightmosts[:length]
	var i int16
	for ; i < length; i++ {
		n.rightmosts[i] = 0
	}
	n.rebuild(0, length-1)
}
...
// DoMatching performs fn on all intervals stored in the tree that overlaps r. A boolean is returned
// indicating whether the traversal was interrupted by an Operation returning true. If fn alters
// stored intervals' sort relationships, future tree operation behaviors are undefined.
...
// visit handles the Interface or child node if it overlaps with r.
func (n *node) visit(fn Operation, r Range, overlapper Overlapper, i int16, ptr *[]int16) (done bool) {
	curR := n.getRange(i)
	if overlapper.Overlap(r, curR) {
		if len(n.children) == 0 {
			return fn(n.interfaces[i])
		}
		if i%2 == 1 {
			return fn(n.interfaces[(i-1)/2])
		}
		slice := *ptr
...
// bst does a binary search over a combined slice of interfaces and children. rightmosts is used for
// guiding the binary search.
func (n *node) bst(fn Operation, r Range, overlapper Overlapper, l, h int16, ptr *[]int16) (done bool) {
	if h-l+1 <= cutoff {
		i := l
		for ; i <= h; i++ {
			if done = n.visit(fn, r, overlapper, i, ptr); done {
				return
			}
		}
		return
	...
	m := (l + h) / 2
	sliceRange := Range{n.getRange(l).Start, n.getRange(n.rightmosts[m]).End}
	if !overlapper.Overlap(r, sliceRange) {
		return
	}
	if done = n.visit(fn, r, overlapper, m, ptr); done {
		return
	}
	if done = n.bst(fn, r, overlapper, l, m-1, ptr); done {
		return
	}
	done = n.bst(fn, r, overlapper, m+1, h, ptr)
	return
}

// doMatch performs fn on intervals stored in the subtree rooted at this node which overlap r.
// TODO(jingguo): make the traversal procceed in the nondecreasing order of interval start.
func (n *node) doMatch(fn Operation, r Range, overlapper Overlapper) (done bool) {
	var l, h int16 = 0, int16(n.allLen() - 1)
	slice := make([]int16, 0, len(n.children))
	ptr := &slice
	done = n.bst(fn, r, overlapper, l, h, ptr)
	if done {
		return
	}
	for _, idx := range *ptr {
		done = n.children[idx].doMatch(fn, r, overlapper)
		if done {
			return
		}
	}
...
func (n *node) stealFromLeftChild(i int, fast bool) {
	...
	if !fast {
		...
		stealFrom.rebuildRightmosts()
		stealTo.rebuildRightmosts()
		n.rebuildRightmosts()
	}
}
...
func (t *Tree) newNode() (n *node) {
	...
	n.t = t
	n.rightmosts = make([]int16, 0, t.maxInterfaces()*2+1)
	return
}

...
func checkFastDelete(t *testing.T, tree *BTree, ivs interfaces, count int) {
...
func checkTraversal(t *testing.T, tree *BTree, ivs interfaces) {
...
func TestLargeTree(t *testing.T) {
	...
	tree := NewBTreeWithDegree(ExclusiveOverlapper, *degree)
	for _, iv := range ivs {
		if err := tree.Insert(iv, true); err != nil {

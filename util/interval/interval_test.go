...
func TestRebuildRightmosts(t *testing.T) {
	ivs := interfaces{
		&Interval{Range{Comparable{1}, Comparable{20}}, 0},
		&Interval{Range{Comparable{3}, Comparable{40}}, 0},
		&Interval{Range{Comparable{5}, Comparable{60}}, 0},
		&Interval{Range{Comparable{7}, Comparable{40}}, 0},
		&Interval{Range{Comparable{9}, Comparable{30}}, 0},
		&Interval{Range{Comparable{11}, Comparable{60}}, 0},
		&Interval{Range{Comparable{13}, Comparable{80}}, 0},
		&Interval{Range{Comparable{15}, Comparable{80}}, 0},
	}
	length := len(ivs)
	tree := NewWithDegree(InclusiveOverlapper, length)
	n := tree.newNode()
	n.interfaces = ivs
	n.rebuildRightmosts()
	if expected := []int16{0, 2, 0, 6, 0, 6, 0, 0}; !reflect.DeepEqual(n.rightmosts, expected) {
		t.Errorf("expected rightmosts %v, got %v", expected, n.rightmosts)
	}
	n.interfaces = n.interfaces[:4]
	n.rebuildRightmosts()
	if expected := []int16{0, 2, 0, 0}; !reflect.DeepEqual(n.rightmosts, expected) {
		t.Errorf("expectecd rightmosts %d, got %v\n", expected, n.rightmosts)
	}
	n.children = children{
		&node{Range: Range{Comparable{0}, Comparable{22}}},
		&node{Range: Range{Comparable{2}, Comparable{23}}},
		&node{Range: Range{Comparable{4}, Comparable{30}}},
		&node{Range: Range{Comparable{6}, Comparable{17}}},
		&node{Range: Range{Comparable{8}, Comparable{21}}},
	}
	n.rebuildRightmosts()
	if expected := []int16{0, 3, 0, 0, 5, 0, 5, 0, 0}; !reflect.DeepEqual(n.rightmosts, expected) {
		t.Errorf("expectecd %d, got %v\n", expected, n.rightmosts)
	}
}
